...
// Iterates through pizza elements on the page and changes their widths based on percentage
...
function updatePositions() {
  ...
  var itemsArrayLen = itemsArray.length;


  for (var i = 0; i < itemsArrayLen; i++) {

    var phase = Math.sin((cachedScrolling / 1250) + phaseArray[0]++);
    var basicMoveLeft = itemsArray[i].basicLeft + 100 * phase;

    itemsArray[i].style.transform = "translateX(" + basicMoveLeft + "px)";
  }
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  for (var i = 0; i < 32; i++) {
    var elemMain = elem.cloneNode(true);
      elemMain.classList.add('mover');
      elemMain.src = "images/pizzasmall.jpg";
      elemMain.style.height = "100px";
      elemMain.style.width = "73.333px";
      elemMain.basicLeft = (i % cols) * s - 500;
      elemMain.style.top = (Math.floor(i / cols) * s) + 'px';
    document.querySelector("#movingPizzas1").appendChild(elemMain);
  }
  updatePositions();
});
